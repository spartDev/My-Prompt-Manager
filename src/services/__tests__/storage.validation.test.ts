...
describe('StorageManager - Import Validation', () => {
  ...
  const expectValidationError = async (importData: string, expectedErrorPattern: string | RegExp) => {
    try {
      ...
      if (!hasMatch) {
        throw new Error(`Expected error matching ${expectedErrorPattern}, but got errors: ${errors.join('; ')}`);
      }
    }
  };
  ...
  describe('Valid Data', () => {
    ...
    it('should accept legacy data without optional fields (backwards compatibility)', async () => {
      ...
      await expect(storageManager.importData(legacyData)).resolves.not.toThrow();
    });
  });
  ...
  describe('Structure Validation', () => {
    ...
    it('should reject null data', async () => {
      await expect(storageManager.importData('null')).rejects.toThrow('Import data must be a valid JSON object');
    });

    it('should reject non-object data', async () => {
      await expect(storageManager.importData('"string"')).rejects.toThrow('Import data must be a valid JSON object');
      await expect(storageManager.importData('123')).rejects.toThrow('Import data must be a valid JSON object');
      await expect(storageManager.importData('true')).rejects.toThrow('Import data must be a valid JSON object');
    });

    it('should reject data missing prompts field', async () => {
      const invalidData = JSON.stringify({
        categories: [{ id: 'cat-1', name: DEFAULT_CATEGORY }],
        settings: { defaultCategory: DEFAULT_CATEGORY, sortOrder: 'updatedAt', sortDirection: 'desc', theme: 'light' }
      });

      await expect(storageManager.importData(invalidData)).rejects.toThrow(/Invalid import data structure.*Missing required field: prompts/);
    });
    ...
    it('should reject data missing categories field', async () => {
      ...
      await expect(storageManager.importData(invalidData)).rejects.toThrow(/Invalid import data structure.*Missing required field: categories/);
    });
    ...
    it('should reject data missing settings field', async () => {
      ...
      await expect(storageManager.importData(invalidData)).rejects.toThrow(/Invalid import data structure.*Missing required field: settings/);
    });
    ...
    it('should reject data where prompts is not an array', async () => {
      ...
      await expect(storageManager.importData(invalidData)).rejects.toThrow(/prompts.*must be an array/);
    });
    ...
    it('should reject data where categories is not an array', async () => {
      ...
      await expect(storageManager.importData(invalidData)).rejects.toThrow(/categories.*must be an array/);
    });
    ...
    it('should reject data where settings is not an object', async () => {
      ...
      await expect(storageManager.importData(invalidData)).rejects.toThrow(/settings.*must be an object/);
    });
  });
  ...
  describe('Prompt Validation', () => {
    ...
    it('should reject prompt missing title', async () => {
      ...
      await expectValidationError(invalidData, 'Missing required field "title"');
    });
    ...
    it('should reject prompt with title exceeding 100 characters', async () => {
      ...
      await expectValidationError(invalidData, 'exceeds maximum length of 100 characters');
    });
    ...
    it('should reject prompt missing content', async () => {
      ...
      await expectValidationError(invalidData, 'Missing required field "content"');
    });
    ...
    it('should reject prompt with content exceeding 20000 characters', async () => {
      ...
      await expectValidationError(invalidData, 'exceeds maximum length of 20,000 characters');
    });
    ...
    it('should reject prompt with negative createdAt', async () => {
      ...
      await expectValidationError(invalidData, 'createdAt" must be a positive number');
    });
    ...
    it('should reject prompt with updatedAt before createdAt', async () => {
      ...
      await expectValidationError(invalidData, 'updatedAt" cannot be before "createdAt"');
    });
    ...
    it('should reject prompt with negative usageCount', async () => {
      ...
      await expectValidationError(invalidData, 'usageCount" cannot be negative');
    });
    ...
    it('should reject prompt with non-integer usageCount', async () => {
      ...
      await expectValidationError(invalidData, 'usageCount" must be an integer');
    });
    ...
    it('should reject prompt with lastUsedAt before createdAt', async () => {
      ...
      await expectValidationError(invalidData, 'lastUsedAt" cannot be before "createdAt"');
    });
  });
  ...
  describe('Category Validation', () => {
    ...
    it('should reject category with name exceeding 50 characters', async () => {
      ...
      const invalidData = JSON.stringify({
        ...
        settings: { defaultCategory: longName, sortOrder: 'updatedAt', sortDirection: 'desc', theme: 'light' }
      });

      await expectValidationError(invalidData, 'exceeds maximum length of 50 characters');
    });
  ...
  describe('Settings Validation', () => {
    ...
    it('should reject settings missing defaultCategory', async () => {
      const invalidData = JSON.stringify({
        prompts: [],
        categories: [{ id: 'cat-1', name: DEFAULT_CATEGORY }],
        settings: { sortOrder: 'updatedAt', sortDirection: 'desc', theme: 'light' }
      });

      await expectValidationError(invalidData, 'Missing required field "defaultCategory"');
    });
    ...
    it('should reject settings missing sortOrder', async () => {
      const invalidData = JSON.stringify({
        prompts: [],
        categories: [{ id: 'cat-1', name: DEFAULT_CATEGORY }],
        settings: { defaultCategory: DEFAULT_CATEGORY, sortDirection: 'desc', theme: 'light' }
      });

      await expectValidationError(invalidData, 'Missing required field "sortOrder"');
    });
    ...
    it('should reject settings with invalid sortOrder', async () => {
      const invalidData = JSON.stringify({
        prompts: [],
        categories: [{ id: 'cat-1', name: DEFAULT_CATEGORY }],
        settings: { defaultCategory: DEFAULT_CATEGORY, sortOrder: 'invalidOrder', sortDirection: 'desc', theme: 'light' }
      });
    ...
    it('should reject settings with invalid sortDirection', async () => {
      const invalidData = JSON.stringify({
        prompts: [],
        categories: [{ id: 'cat-1', name: DEFAULT_CATEGORY }],
        settings: { defaultCategory: DEFAULT_CATEGORY, sortOrder: 'updatedAt', sortDirection: 'sideways', theme: 'light' }
      });
    ...
    it('should reject settings with invalid theme', async () => {
      const invalidData = JSON.stringify({
        prompts: [],
        categories: [{ id: 'cat-1', name: DEFAULT_CATEGORY }],
        settings: { defaultCategory: DEFAULT_CATEGORY, sortOrder: 'updatedAt', sortDirection: 'desc', theme: 'rainbow' }
      });
    ...
    it('should reject settings with invalid interfaceMode', async () => {
      const invalidData = JSON.stringify({
        prompts: [],
        categories: [{ id: 'cat-1', name: DEFAULT_CATEGORY }],
        settings: { defaultCategory: DEFAULT_CATEGORY, sortOrder: 'updatedAt', sortDirection: 'desc', theme: 'light', interfaceMode: 'holographic' }
      });
    ...
    it('should accept all valid sortOrder values', async () => {
      const validOrders = ['createdAt', 'updatedAt', 'title', 'usageCount', 'lastUsedAt'];

      for (const sortOrder of validOrders) {
        const validData = JSON.stringify({
          prompts: [],
          categories: [{ id: 'cat-1', name: DEFAULT_CATEGORY }],
          settings: { defaultCategory: DEFAULT_CATEGORY, sortOrder, sortDirection: 'desc', theme: 'light' }
        });
  ...
  describe('Duplicate Detection', () => {
    ...
    it('should reject data with duplicate category names (case-insensitive)', async () => {
      const invalidData = JSON.stringify({
        ...
        settings: { defaultCategory: 'Test', sortOrder: 'updatedAt', sortDirection: 'desc', theme: 'light' }
      });
  ...
  describe('Referential Integrity', () => {
    ...
    it('should reject data where prompts reference non-existent categories', async () => {
      ...
      await expectValidationError(invalidData, 'Referential integrity violation');
    });
  ...
  describe('Multiple Errors', () => {
    it('should collect and report all validation errors', async () => {
      const invalidData = JSON.stringify({
        ...
        categories: [{ id: '', name: '' }],
        settings: { defaultCategory: DEFAULT_CATEGORY, sortOrder: 'invalid', sortDirection: 'invalid', theme: 'invalid' }
      });
  ...
  describe('Corrupted Sample Files', () => {
    ...
    it('should reject missing-fields.json', async () => {
      const fileContent = readFileSync(join(samplesDir, 'missing-fields.json'), 'utf-8');
    ...
    it('should reject wrong-types.json', async () => {
      const fileContent = readFileSync(join(samplesDir, 'wrong-types.json'), 'utf-8');
    ...
    it('should reject duplicate-ids.json', async () => {
      const fileContent = readFileSync(join(samplesDir, 'duplicate-ids.json'), 'utf-8');
    ...
    it('should reject broken-references.json', async () => {
      const fileContent = readFileSync(join(samplesDir, 'broken-references.json'), 'utf-8');
      await expectValidationError(fileContent, 'Referential integrity violation');
    });

     

    it('should reject invalid-timestamps.json', async () => {
      const fileContent = readFileSync(join(samplesDir, 'invalid-timestamps.json'), 'utf-8');
      await expectValidationError(fileContent, /(createdAt|updatedAt|usageCount|lastUsedAt)/);
    });

     

    it('should reject invalid-settings.json', async () => {
      const fileContent = readFileSync(join(samplesDir, 'invalid-settings.json'), 'utf-8');
    ...
    it('should reject length-violations.json', async () => {
      const fileContent = readFileSync(join(samplesDir, 'length-violations.json'), 'utf-8');
