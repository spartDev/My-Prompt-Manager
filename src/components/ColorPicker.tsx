...
const ColorPicker: FC<ColorPickerProps> = ({
  ...
  const handleCustomColorChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (isValidHexColor(color)) {
      setCustomColor(color);
      onChange(color);
      setIsOpen(false); // Close dropdown after selecting from native picker
    }
  };
  ...
  const handleCustomColorSubmit = () => {
    if (isValidHexColor(hexInputValue)) {
      ...
      setShowCustomInput(false);
      setIsOpen(false); // Close dropdown after submitting custom color
    }
  };
      ...
      <div className="flex flex-col space-y-2">
        ...
        <Dropdown.Root open={isOpen} onOpenChange={setIsOpen}>
          ...
          <Dropdown.Content
            ...
            align="start"
            sideOffset={4}
            className="w-72 bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-purple-100 dark:border-gray-700 p-4"
          >
            {/* Preset Colors Grid */}
            <div className="mb-4">
            ...
            {/* Divider */}
            <Dropdown.Separator className="my-4" />
